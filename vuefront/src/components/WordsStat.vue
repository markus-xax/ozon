...
// import PhraseCellRenderer from './PhraseCellRenderer.js'
...
export default {
  ...
  components: {
    AgGridVue,
    MyPhraseCellRenderer: {
      template: '<span>{{ getValue() }}</span>',
      data: () => ({
        valuePhrase: '12331123'
      }),
      methods: {
        getValue () {
      ...
      beforeMount () {
        console.log('beforemounted', this.params)
      }
    }
  },
  props: {
    ...
    rows () {
      ...
      if (this.words && this.words.length) {
        this.words.forEach((item) => {
          ...
          words.push({id: item.id, word: item.word, count: item.count, results: item.results, ...datesObj})
        })
      }
      return words
    },
  ...
  beforeMount () {
    ...
    this.columns = [
      { headerName: 'Ключевое слово', field: 'word', sortable: true, pinned: 'left'/*, cellRendererFramework: 'MyPhraseCellRenderer'*/
      },
