<template>
  ...
  >
    ...
    <v-card>
      ...
      <v-card-text>
        <div class="d-flex input-wrap-full"
             v-if="!foundSku"
        >
          ...
          >
            ...
            <input type="text"
                   class="pl-3 pr-3"
                   placeholder="https://www.wildberries.ru/catalog/2389150/detail.aspx или 23891501"
                   v-model="projectUrl"
                   @change="seekBySku"
                   ref="skusearch"
            >
...
import AgPreloader from './AgPreloader.vue'

export default {
  name: "DialogSeoSkuAdd",
  components: {
    AgPreloader
  },
  data: () => ({
  ...
  methods: {
    async addSkuToProject () {
      ...
      await this.addSku(projectId, {
        ...
        console.log('addtoproject', response)
        if (response.data.status >= 400) {
          that.$snotify.error(response.data.error)
        } else {
          that.$snotify.success("Данные успешно обновлены")
          this.dialog = false
        }
      }).catch((err) => {
        ...
        try {
          await this.searchSku(sku.pop()).then(async (response) => {
            ...
            if (data.status !== 200) {
              ...
              console.log(that.loadingSearch)
              return false
            }
            return this.sku.sku
          }).then((sku) => {
            ...
            this.foundSku = true
            console.log('fetchWords sku', sku)
            let that = this
    ...
    ...mapActions(useSeoProjectsStore, ['setFilterByName', 'searchSku', 'resetSku', 'fetchLimits', 'fetchWords', 'addSku'])
  },
  watch: {
